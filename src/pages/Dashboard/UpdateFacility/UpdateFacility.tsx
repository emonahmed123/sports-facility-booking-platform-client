...
import { useGetSingleFacilityQuery, useUpdateSingleFacilityMutation } from '@/redux/api/facilitesApi/facilitesApi';
...
const UpdateFacility = () => {
    ...
    const [UpdateFacility,{isError ,error}]=useUpdateSingleFacilityMutation()
      ...
      const onSubmit = async (data: any) => {
        ...
        for (const key in data) {
            if (data[key] !== undefined && data[key] !== '') {
              filteredData[key] = key === 'pricePerHour' ? Number(data[key]) : data[key];
            }
          }
    ...
    return (
      ...
      <form className="mx-45" onSubmit={handleSubmit(onSubmit)}>
        <h1 className="text-[40px] leading-[50px] my-5">Update {name} Facility</h1>
