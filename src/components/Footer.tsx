import React from 'react';

const Footer = () => {
    return (
        <footer>
        <div className="pt-[70px] pb-[30px] bg-[#FFFFFF] font-Poppis">
          <div className="max-w-[1170px] mx-auto px-5 xl:px-0">
            <div
              className="flex lg:items-start justify-between flex-col lg:flex-row gap-y-[60px] mb-[30px]"
            >
              <div className="max-w-[370px]">
                <a className="mb-[40px]" href="">
                  <img
                    src="https://besnik-space.fra1.cdn.digitaloceanspaces.com/ezytor/theme/N8lApmzhKXbK5HA7kTpU1702703250.svg"
                    alt=""
                /></a>
      
                <p className="text-16px leading-28px mt-[30px] text-[#2E382C]">
                Find and Book the Best Sports Facilities
                Your gateway to top-tier sports venues—anytime, anywhere.s
                </p>
              </div>
      
              <div className="max-w-[723px]">
                <div
                  className="flex gap-x-[60px] lg:gap-x-[110px] justify-between flex-wrap"
                >
                  <div>
                    <h5
                      className="text-20px leading-30px font-bold text-[#3d85ff] mb-[24px]"
                    >
                      Company
                    </h5>
      
                    <ul>
                      <li className="mb-[18px]">
                        <a className="text-[16px] leading-26px text-[#2E382C]" href=""
                          >Careers</a
                        >
                      </li>
                      <li className="mb-[18px]">
                        <a className="text-[16px] leading-26px text-[#2E382C]" href=""
                          >Press</a
                        >
                      </li>
                      <li className="mb-[18px]">
                        <a className="text-[16px] leading-26px text-[#2E382C]" href=""
                          >About us</a
                        >
                      </li>
                    </ul>
                  </div>
                  <div>
                    <h5
                      className="text-20px leading-30px font-bold text-[#3d85ff] mb-[24px]"
                    >
                     About Us
                    </h5>
      
                    <ul>
                      <li className="mb-[18px]">
                        <a className="text-[16px] leading-26px text-[#2E382C]" href=""
                          >Blog</a
                        >
                      </li>
                    
                      <li className="mb-[18px]">
                        <a className="text-[16px] leading-26px text-[#2E382C]" href=""
                          >Community</a
                        >
                      </li>
                      <li className="mb-[18px]">
                        <a className="text-[16px] leading-26px text-[#2E382C]" href=""
                          >FAQ</a
                        >
                      </li>
                    </ul>
                  </div>
                  <div>
                    <h5
                      className="text-20px leading-30px font-bold text-[#3d85ff] mb-[24px]"
                    >
                      Contact Us
                    </h5>
      
                    <ul>
                      <li className="mb-[18px]">
                        <a className="text-[16px] leading-26px text-[#2E382C]" href=""
                          >LinkedIn</a
                        >
                      </li>
                      <li className="mb-[18px]">
                        <a className="text-[16px] leading-26px text-[#2E382C]" href=""
                          >Facebook</a
                        >
                      </li>
                      <li className="mb-[18px]">
                        <a className="text-[16px] leading-26px text-[#2E382C]" href=""
                          >Instagram</a
                        >
                      </li>
                      <li className="mb-[18px]">
                        <a className="text-[16px] leading-26px text-[#2E382C]" href=""
                          >Twitter</a
                        >
                      </li>
                    </ul>
                  </div>
                </div>
              </div>
            </div>
      
            <div className="max-w-[1170px] mx-auto h-[1px] bg-[#3d85ff] mb-[30px]"></div>
      
            <div className="flex justify-between flex-col md:flex-row gap-y-[30px]">
              <div className="flex gap-x-[15px] sm:gap-x-[30px]">
                <a className="text-[14px] leading-24px text-[#2E382C]" href=""
                  >Terms of Service</a
                >
                <a className="text-[14px] leading-24px text-[#2E382C]" href=""
                  >Privacy Policy</a
                >
                <a className="text-[14px] leading-24px text-[#2E382C]" href=""
                  >Cookie Policy</a
                >
              </div>
              <p className="text-[14px] leading-24px text-[#2E382C]">
                Copyright © 2024 Company All Rights Reserved
              </p>
            </div>
          </div>
        </div>
      </footer>
    );
}; 


export default Footer;